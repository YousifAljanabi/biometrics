...
def load_pair(fname, distorted_dir, clean_dir):
    ...
    # Check if filename has variation suffix (_varXX)
    has_var_suffix = tf.strings.regex_full_match(name_no_ext, r".*_var\d+$")
...
def make_dataset(distorted_dir, clean_dir, cond_map=None, shuffle=True, drop_remainder=True):
    # Get all filenames from distorted directory (with subfolder structure)
    all_fnames = []
        ...
        if tf.io.gfile.isdir(distortion_path):
            ...
            for fname in folder_files:
                if fname.lower().endswith((".png", ".jpg", ".jpeg", ".bmp")):
                    # Add relative path: distortion_folder/filename
                    relative_path = f"{distortion_folder}/{fname}"
                    all_fnames.append(relative_path)
